...
export function UsersFilterBar({ defaultSearch, defaultRole, defaultVerified }: UsersFilterBarProps) {
    ...
    const handleVerifiedChange = (value: string) => {
        startTransition(() => {
            router.push(`${pathname}?${createQueryString({ verified: value === "ALL" ? "" : value })}`);
        });
    };
    ...
    const hasActiveFilters = defaultSearch || defaultRole || defaultVerified;
    ...
    return (
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            {/* Verified Filter */}
            <Select
                defaultValue={defaultVerified || "ALL"}
                onValueChange={handleVerifiedChange}
                disabled={isPending}
            >
                <SelectTrigger className="w-[160px] border-2 border-slate-200 rounded-xl h-9 font-medium">
